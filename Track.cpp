...
CTrack::CTrack()
{
	...
	m_oGotInstrument = false; //IX - Added
}
...
// Convert a buzz note to a straight midi style note-number
float CTrack::ConvertNote(unsigned int buzzNote)
{
	// #define	CONVNOTE(x) ( (!NONOTE(x)) ? ((x)>>4)*12+((x)&0xF)-1 : (x ))
	// if not a note
	if((buzzNote==NOTE_NO) || (buzzNote==NOTE_OFF))
	{
		return (float)buzzNote;
	} else {
		// divide the buzz note into octaves/note
		unsigned int octave = (buzzNote)>>4;
		unsigned int note = (buzzNote & 0x0F)-1;

		// then map through the pitches part
		return (12.0f * octave) + note % 12;
	}
}

// Convert a buzz note to a floating point note number
// incorporating the detuning attributes
float CTrack::ConvertTunedNote(unsigned int buzzNote)
{
	// if not a note
	if((buzzNote==NOTE_NO) || (buzzNote==NOTE_OFF))
	{
		...
		// then map through the pitches part
		return (12.0f * octave) + m_pMachine->m_fNotePitches[note % 12];	// make sure note isn't higher than 12
	}
}
...
int	CTrack::NewNote( bool oRetrig )
{
	...
	if( m_pChannel==NULL )
	{
		m_pChannel=m_pMachine->AllocChannel(m_Output);
	...
	if( m_pSample )
	{
		...
		if( !oRetrig )
		{
			...
			m_fFreq      =(float) pow(2.0f,(ConvertTunedNote(m_iBaseNote)-ConvertNote(m_pSample->GetRootNote()))/12.0f) * m_pSample->GetRootFrequency() / m_pMachine->pMasterInfo->SamplesPerSec;
			m_fBaseFreq  =m_fFreq;
...
void	CTrack::Tick( CTrackVals &tv, CGlobalVals &gv )
{
	...
	if( m_Vals.instrument> 0 || !m_oGotInstrument)
	{
		...
		//IX - Altered and moved
		if( m_Vals.instrument > 0)
		{
		...
		//IX - Added this test
		m_oGotInstrument = m_pChannel->m_pInstrument ? true : false;
		
		//if( m_pSample && !CMatilde::s_oVirtualChannels )
		//	m_pSample->Free();
		//m_pSample=NULL;
	...
	for( i=0; i<TOTALEFFECTS; i+=1 )
	{
		...
		switch (command)
		{
			...
			case 0x16:
				if( argument )
				{
					m_fFreq*=(float)pow(2.0f,float((rand()%(argument<<1))-argument)/FREQSCALE);
					retrig|=RETRIG_FREQ;
				}
				break;
			case 0x17:
			...
			case 0xE4:
				...
				break;
			case 0xE5:
				m_fFinetune=float(pow( pow(2.0f,1.0f/12.0f), (argument-128)/256.0f ));
			case 0xE6:
...
//				if( m_pChannel->m_pWaveInfo->Flags&WF_STEREO )
				...
				else
				{
					...
					m_pChannel->m_Resampler.m_Loop.m_eFormat = m_pChannel->m_Resampler.m_Location.m_eFormat;
